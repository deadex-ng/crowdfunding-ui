...
function HomePage() {
  ...
  const donate = async () => {
    console.log("YOU CLIKED:",id)
    const resp = await makeDonation(id,userAccount)
    console.log("DOnATion RESUlts: ",resp)
  }
              ...
              <tbody >
                {data.map(
                  (info, index) => {
                    ...
                    console.log("count value is:", campaignAccounts[count])
                    to = campaignAccounts[count]
                    console.log("You would send to:", to)
                    count++
                    var ipfsLink = "https://ipfs.io/ipfs/" + info.hash
                    console.log("Accounts List", campaignAccounts)
                    return (
